...
@Component({
  selector: 'app-usr-dash-board',
  templateUrl: './usr-dash-board.component.html',
  styleUrls: ['./usr-dash-board.component.scss']
})
export class UsrDashBoardComponent implements OnInit {
  constructor(private loginservice:LogInServiceService,private router: Router, private filedownload: FileDownloadService, private FileService: TscGenerateService, private actionAlert: ActionAlertService) { }
  ...
  ngOnInit(): void {
    this.usrId =this.loginservice.usr.id
  ...
  private updateRows(data: TscFile[]) {
    ...
    for (let i = 0; i < data.length; i++) {
      this.files[i] = data[i];
      console.log(this.usrId+" "+ this.files[i].user.id)
     
      if(this.usrId==this.files[i].user.id&&this.files[i].status){
    ...
    if (this.range.end >= this.range.start) {
      ...
      if (!!this.productSelected && !!this.releaseSelected) {
        ...
        if(!!this.range.end&&!!this.range.start&&!this.dateflag){
          ...
          let usrId= this.loginservice.usr.id
          this.FileService.viewFilesByUserIdAsync(endD,this.productSelected.id,this.releaseSelected.id,startD,usrId).subscribe(data=>{
            this.searchRelease=[];
